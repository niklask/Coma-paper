...
to be $< 15\%$ from VERITAS data and $< 2.5\%$ from \Fermi data (averaged within the virial
...
\C{(99\%\ CL)} strongly depend on $\alpha$. This is due to the comparably large \C{energy range from
  GeV energies (that dominate the CR pressure, provided $\alpha>2$ and a non-relativistic
  low-momentum cutoff, i.e., $q<m_p c$) to energies at 220 GeV, where our quality selection criteria
  imposed the energy threshold. These gamma-ray energies correspond to 1.6 TeV CRs -- an energy
  ratio of more than 3 orders of magnitudes which explains the sensitivity to small changes in
  $\alpha$.} The flux measurements within 0.2$^{\circ}$ are the most constraining due to a
competition between integrated signal and background for an increasing size of the radius of
...
emission at GHz frequencies is dominated by the electrons with an energy $E_\rmn{e} \sim 2.5$ GeV
...
\caption{Integral flux upper limits for Coma (this work, Tables~\ref{table:results} and
  \ref{table:fermi}) for different integration radii (shown with different grey scales), assuming
  ...
  protons (dotted). Note that the \Fermi limit for the energy interval of 1--3 GeV within the
...
\colhead{RoI\tablenotemark{a} [deg]} 
    ...
    & \colhead{$F_{\gamma,\rmn{sim}}(E)$\tablenotemark{c}} & \colhead{$F_{\gamma,\rmn{sim}}(>E)$\tablenotemark{d}}   \\
