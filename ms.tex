...
The gamma-ray flux due to self-annihilations of WIMPs is given by 
\begin{equation}
\frac{dN(\Delta\Omega)}{dE}=\frac{\expval{\sigma v}}{8\pi m_{\chi}^{2}}\left[\frac{dN(E,m_{\chi})}{dE}\right]\int_{\Delta\Omega}d\Omega\int d\lambda \rho^{2}(\lambda,\Omega),
\label{eqn:WIMPflux}
\end{equation}
where $\expval{\sigma v}$ is the thermally averaged product of the total self-annihilation cross section and the WIMP velocity, $m_{\chi}$ is the WIMP mass, $dN(E,m_{\chi})/dE$ is the gamma-ray yield per annihilation, $\Delta\Omega$ is the observed solid angle, $\rho$ is the DM mass density, and $\lambda$ is the line-of-sight distance to the integration volume.

The two integrals in Eq. (\ref{eqn:WIMPflux}) results in a dimensionless factor representing the astrophysical contribution to the flux
\begin{equation}
J(\Delta\Omega)=\left(\frac{1}{\rho_{c}^{2}R_{H}}\right) \int_{\Delta\Omega}d\Omega\int d\lambda \rho^{2}(\lambda,\Omega),
\end{equation}
which here has been normalized to the square of the critical density, $\rho_{c}$, and the Hubble radius, $R_{H}$.

Given the upper limit on the observed gamma-ray rate, $R_{\gamma}(95\%\mathrm{CL})$, we can place constraints on the WIMP parameter space $(m_{\chi}, \expval{\sigma v})$. 
