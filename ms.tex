...
combination with their long cooling time which exceeds the Hubble time, this suggests they may
also be sources of very high-energy (VHE) gamma-ray emission as a result of highly relativistic
...
respectively. For VERITAS ({\em Fermi}), the limits are a factor of 60 (20) higher than
...
limits to constrain the populations of CRs and magnetic fields and in Coma. Using an analytical
approach informed by cosmological cluster simulations, the CR-to-thermal pressure is constrained
to be $< 15\%$ from VERITAS data and $< 2.5\%$ from {\em Fermi} data (averaged within the virial
...
$0.5$--$3\mu$G, depending on the magnetic decline and gamma-ray spectral index.  Since these
values are well below those inferred by Faraday rotation measurements in Coma, this makes the
hadronic model a very plausible explanation of the Coma radio halo. Finally, a constraint on the
thermally averaged product of the total self-annihilation cross section and the relative velocity
of the dark-matter particles, $\expval{\sigma v}$, is derived and found to be $\lesssim 10^{-21}$
cm$^{3}$ s$^{-1}$ for $m_{\chi}\gtrsim 1000$ GeV/c$^{2}$.
\end{abstract}
...
\citep[see][for a review]{article:Voit:2005}. Most of the mass ($\sim$80\%) in a cluster is in form
of dark matter (DM), as indicated by galaxy dynamics and gravitational lensing
...
e.g.,][]{article:ColafrancescoBlasi:1998, article:Ryu_etal:2003}. Many clusters feature
...
in the ICM is an efficient process, a population of highly relativistic CR protons and heavy ions
is to be expected in the ICM. The main energy-loss mechanism for CR hadrons at high energies is pion
...
scales of a cluster and the volume-filling magnetic fields in the ICM, CR hadrons will be confined
in the cluster on timescales comparable to or longer than the Hubble time
\citep[][]{article:Volk_etal:1996, article:Berezinsky_etal:1997} allowing them to accumulate over
time. For a given CR distribution function, the hadronically induced gamma-ray flux is directly
proportional to the CR-to-thermal pressure fraction, $X_\CR=\expval{P_{\CR}}/
\expval{P_{\mathrm{th}}}$. Already a very modest $X_{\CR}$ of a few percent implies an observable
flux of high-energy gamma rays \citep[e.g.,][]{article:PfrommerEnsslin:2004b}. 
...
a detection of a two-phase structure of CRs and the thermal ICM. A fraction $X_\CR$ of only a few 
percent is required in order to produce a gamma-ray flux observable with current generation of
gamma-ray telescopes rendering this technique at least as sensitive as the dynamical and
hydrostatic method (which are more general in that they are sensitive to any non-thermal pressure
component).
...
background. It extends out to a radius of 0.42 square degrees to cover all regions of interest
...
spatial CR distribution that is just consistent with the flux upper limits and requiring the model
to match the observed radio data, this enables us to derive a lower limit on the magnetic field
...
fluxes (summarized in Table \ref{table:constraints_simple}) are compared to {\em Fermi} and VERITAS
flux upper limits of the same energy intervals.  The gamma-ray flux limit of {\em Fermi} in the
...
parameters as laid out above.  In our further analysis, we use the most constraining {\em Fermi}
...
appears necessary to explain the radio halo bimodality). (3) We finally use a pragmatic
