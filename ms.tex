...
\section{Results}
No significant excess of VHE gamma-rays from the Coma cluster was detected with VERITAS. 

A 99\% confidence level upper limit is calculated for each region of interest using the method described by \citet{article:Rolke:2005} assuming a Gaussian background. Figure \ref{fig:sigdist} shows that the VERITAS data is well fit by a Gaussian with a mean of zero and unit standard deviation. The results are listed in Table \ref{table:results}.
